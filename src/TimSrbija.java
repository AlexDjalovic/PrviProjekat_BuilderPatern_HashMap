...
public class TimSrbija extends KeteringTim {
...
double p,d;
	...
	public void postaviKomponente() {
		...
		//POSTAVLJANJE VREDNOSTI U COMBU ZA sRBIJU IZ HASHMAPA
				//pozivom f3.getSrpskaPredjela pozivamo mapu, a sa .keySet prolazimo Kroz kljuceve koji
				//su String , da bi ih stavili u kombove
				for(String x:f3.getSrpskaPredjela().keySet()){
					predCombo.addItem(x);
				}
				//ista prica vazi za dezerte
				for(String y:f3.getSrpskiDezerti().keySet()){
					dezCombo.addItem(y);
				}
	}
	@Override
	public String prikaziNaArea() {
		...
		if(f3.srpskiDezerti.containsKey(dezert1))  {d=f3.srpskiDezerti.get(dezert1);}
		if(f3.srpskaPredjela.containsKey(predjelo1)){p=f3.srpskaPredjela.get(predjelo1);}
		//ovo je zbir 2 cene
		double total=p+d;
